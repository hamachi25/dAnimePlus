...
function qualityAndYear(torf) {
    ...
    for (let i = 0; i < playerMypage.length; i++) {
        const workId = new URL(
            document.querySelectorAll(".textContainer[href]")[i]
...
if (path == "mpa_fav_pc" || path == "mpa_hst_pc") {
    ...
    for (let i = 0; i < playerMypage.length; i++) {
        ...
        document
            .querySelectorAll(".itemModule > section")
        [i].insertAdjacentHTML("afterbegin", header);
    }
    ...
    const observer = new MutationObserver(() => {
        ...
        if (resolutionbool) {
            ...
            const fetchAsync = async () => {
                const url =
                    "https://animestore.docomo.ne.jp/animestore/rest/v1/works?work_id=" +
                    workIds.join(",");
                const response = await fetch(url);
                const json = await response.json();
                const [sorted, yearSorted] = sort(workIds, json);
    ...
    document.addEventListener("mousemove", () => {
        if (isDragging) {
            ...
            document.querySelector("#volumeText > span").textContent = Math.round(
                video.volume * 100
            );
        }
    });
    ...
    if (resolutionbool) {
        const observer = new MutationObserver(() => {
            ...
            let workIds = [];
            for (let i = 0; i < playerMypage.length; i++) {
                const workId = new URL(playerMypage[i].getAttribute("href")).searchParams.get(
                    "workId"
                );
                workIds.push(workId);
            }
